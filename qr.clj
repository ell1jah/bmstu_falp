...
(defn get-projection [arr1 arr2]
  (map 
   #(* % (/
          (get-dot-product arr1 arr2)
          (* (get-norm arr2) (get-norm arr2))))
   arr2))
   ...
   (nth 
    (reduce 
     ...
     (concat
      [(get-column a j)]
      (map
       (fn [x] (get-projection (get-column a j) x))
       (q-cols a j))))
    i) 
   (r-elem a j j)))
...
(defn qr-j-rec [a elem i j]
  (cond
    (< j 0) nil
    (>= j 0) (concat
              (qr-j-rec a elem i (dec j))
              (list (elem a i j)))))

(defn qr-i-rec [a elem i]
  (cond
    (= i 0) (list
             (qr-j-rec a elem i (dec (count (first a)))))
    (> i 0) (concat
             (qr-i-rec a elem (dec i))
             (list
              (qr-j-rec a elem i (dec (count (first a))))))))

(defn qr [a]
  (list
   (qr-i-rec a q-elem (dec (count a)))
   (qr-i-rec a r-elem (dec (count (first a))))))
